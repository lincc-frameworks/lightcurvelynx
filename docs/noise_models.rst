...
LightCurveLynx allows the user to select which noise model to use for the simulation, by including `FluxNoiseModel` object within the `SurveyInfo` object passed to the simulator. As sdescribed below, different noise models can be applied by using different subclasses of the `FluxNoiseModel` base class.
