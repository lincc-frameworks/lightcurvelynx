...
class PoissonFluxNoiseModel(FluxNoiseModel):
    ...
    where the columns vary. Subclass should override the extract_noise_features
    method.
    """
    ...
    def compute_flux_error(self, bandflux, obs_table, indices):
        ...
        total_exposure_time = obs_table.get_value_per_row(
            "exptime",
            indices=indices,
        )
        exposure_count = obs_table.get_value_per_row(
            "nexposure",
            indices=indices,
            default=1,
        )
        sky = obs_table.get_value_per_row(
            "sky_bg_e",
            indices=indices,
        )
        psf_footprint = obs_table.get_value_per_row(
            "psf_footprint",
            indices=indices,
        )
        zp = obs_table.get_value_per_row(
            "zp",
            indices=indices,
        )
        readout_noise = obs_table.get_value_per_row(
            "read_noise",
            indices=indices,
        )
        dark_current = obs_table.get_value_per_row(
            "dark_current",
            indices=indices,
        )
        zp_err_mag = obs_table.get_value_per_row(
            "zp_err_mag",
            indices=indices,
            default=0.0,
        )
